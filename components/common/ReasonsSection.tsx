...
function ReasonsSection() {
  return (
    <section className='w-full bg-black h-fit py-8 md:py-16 lg:py-28  px-2 md:px-8 lg:px-16 text-white flex flex-col items-center justify-center gap-12'>
      ...
      <div className='flex flex-col md:flex-row w-[80%] items-center justify-between gap-4 lg:gap-12'>
        {reasonsItems.map(item => (
          <div
            key={item.title}
            className='flex flex-col justify-between items-center gap-4 text-center p-4 md:p-8 xl:px-12 h-full w-full'
          >
            <h1 className='md:text-lg lg:text-xl text-center uppercase'>
              {item.title}
            </h1>
            <div className='text-primary hover:text-white bg-white/30 hover:bg-primary/60 w-fit h-fit p-4 md:p-8 rounded-full hover:scale-110 hover:shadow-md hover:shadow-gray-200'>
              {item.icon}
            </div>
